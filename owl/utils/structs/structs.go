...
func (s *Struct) Map(style KeyStyle) map[string]interface{} {
...
// given map.
func (s *Struct) FillMap(out map[string]interface{}, style KeyStyle) {
	if out == nil {
		return
	}
	...
	for _, field := range fields {
		...
		var name string
		if style == Pascal {
			name = strutil.Udl2UCC(field.Name)
		} else if style == Underline {
			name = strutil.Cc2Udl(field.Name)
		} else if style == CamelCase {
			name = strutil.Udl2UCC(field.Name)
		} else if style == LowerCamelCase {
			name = strutil.Udl2LCC(field.Name)
		} else {
			name = field.Name
		}
...
func Map(s interface{}, style KeyStyle) map[string]interface{} {
	return New(s).Map(style)
}

// FillMap is the same as Map. Instead of returning the output, it fills the
// given map.
func FillMap(s interface{}, out map[string]interface{}, style KeyStyle) {
	New(s).FillMap(out, style)
}
...
// nested value.
func (s *Struct) nested(val reflect.Value, style KeyStyle) interface{} {
	var finalVal interface{}
	...
	case reflect.Map:
		...
		if mapElem.Kind() == reflect.Struct ||
			...
			for _, k := range val.MapKeys() {
				m[k.String()] = s.nested(val.MapIndex(k), style)
			}
			finalVal = m
			break
		}
	...
	case reflect.Slice, reflect.Array:
		...
		for x := 0; x < val.Len(); x++ {
			slices[x] = s.nested(val.Index(x), style)
		}
		finalVal = slices
	default:
		finalVal = val.Interface()
